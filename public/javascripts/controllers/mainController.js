...
angular.module('ethExplorer')
    .controller('mainCtrl', function ($rootScope, $scope, $location) {
        ...
        function updateStats() {
            ...
            if($scope.blockNumber!==undefined){
                ...
                if(blockNewest!==undefined){
                    ...
                    $scope.AltsheetsCoefficient = $scope.totalDifficultyDividedByDifficulty.dividedBy($scope.blockNumber);
                    ...
                    var blockBefore = web3.eth.getBlock($scope.blockNumber - 1);
                    if(blockBefore!==undefined){
                        $scope.blocktime = blockNewest.timestamp - blockBefore.timestamp;
                    }
                    $scope.range1=100;
                    range = $scope.range1;
                    var blockPast = web3.eth.getBlock(Math.max($scope.blockNumber - range,0));
                    if(blockBefore!==undefined){
        ...
        function updateTXList() {
            ...
            $scope.txNumber = currentTXBlockNumber - 1;
            $scope.recenttransactions = [];
            for (var i = 0; i < 10 && currentTXBlockNumber - i >= 0; i++) {
                $scope.recenttransactions.push(web3.eth.getTransactionFromBlock(currentTXBlockNumber - i));
            }
        ...
        function updateBlockList() {
            ...
            $scope.blockNumber = currentBlockNumber - 1;
            $scope.blocks = [];
