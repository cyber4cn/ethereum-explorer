        ...
        $scope.init=function()
        {
            ...
            if($scope.txId!==undefined) { // add a test to check if it match tx paterns to avoid useless API call, clients are not obliged to come from the search form...
                ...
                getTransactionInfos()
                    ...
                    if(result.input.length > 74)
                    {
                        $scope.toAddress = '0x' + result.input.substr(34, 40);
                    }

                    var toAmount = result.input.substr(74);

                    //= hexdec(preg_replace('/^0+/', '', )) /// Math.pow(10, $contractDecimal);

                    $scope.toAmount = toAmount.replace(/^0+/g,'');;
